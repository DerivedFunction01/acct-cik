...
## Execute SELECT Statements on report data
...
dd = execute_sql("DELETE FROM report_data WHERE url=''", fetch=False)
print(dd)
```
...
```{python}
ff = execute_sql("SELECT * FROM names", fetch=True)
ff[["name"]].to_excel("../excel/names.xlsx", index=False)
ff.head()
```
