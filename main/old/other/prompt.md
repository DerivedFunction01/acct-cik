...
If the model is wrong for a specific sentence, that is okay, as long it gets it right at some point for that firm year. Just flag it in a separate table if the model is wrong and did not get it right overall.  The flags "TERMINATED" and "NO" mean the same thing, since overall we want to verify actual usage.
