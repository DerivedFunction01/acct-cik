...
def generate_hedge_paragraph(
    ...
    def generate_derivative_sentences(year_to_use: int) -> list[str]:
        ...
        if has_active_derivative:
            ...
            notional = 0 if year == base_year else generate_value()
        ...
        future_year = (
            ...
            else random.randint(old_year - 1, base_year - 1)
        )
    ...
    def hedge_type_policy(year_to_use: int, additional=False) -> list[str]:
        ...
        if not has_active_derivative and random.random() < 0.45 and additional:
    ...
    if has_active_derivative is None: # Speculative / Policy-only
        ...
        if (include_policy or mixed) and len(cleanup(all_sentences, base_year)) < 400:
