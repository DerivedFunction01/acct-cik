...
from derivative_regex import (
    ...
    IR_CONTEXT_REGEX, FX_CONTEXT_REGEX, CP_CONTEXT_REGEX, EQ_CONTEXT_REGEX,
...
# =============================================================================
DB_PATH = "web_data.db"
OUTPUT_PATH = "roberta/classification_data_v14_pure.parquet"

# REMOVED EXTERNAL PATHS (Pure DB only)
...
class ContextScorer:
    def score(self, text: str, label: str) -> int:
        ...
        if re.search(r"\b(hedg|mitigat|manag)(?:e|es|ed|ing)\b", text, re.I): score += 15
        
        if label == 'ir' and re.search(r"\b(variable|floating|fixed)\s+rate\b", text, re.I): score += 20
        if label == 'fx' and re.search(r"\b(foreign|exchange)\s+rate\b", text, re.I): score += 20
        if label == 'cp' and re.search(r"\b(price|commodity|fuel|oil)\b", text, re.I): score += 20
        
        if label == 'eq':
            text_lower = text.lower()
            is_comp_talk = any(x in text_lower for x in ['employee', 'compensation', 'vesting', 'grant', 'award'])
            is_hedging_talk = re.search(r"\b(hedg|mitigat|manag|offset)(?:e|es|ed|ing)\b", text, re.I)
            if is_comp_talk and not is_hedging_talk: return -1 
            if is_comp_talk and is_hedging_talk: score += 25 
                
        return score
    
    def get_max_score_any_category(self, text: str) -> int:
        """Returns the highest context score found for ANY category."""
...
class AugmentationEngine:
    def __init__(self):
        self.generic_terms = [
            "derivative", "instrument", "contract", "agreement", 
            "hedging instrument", "hedge", "position"
        ]
        self.bases = [b.replace("?", "").replace("s", "") for b in UNAMBIGUOUS_BASE_TYPES]
...
class DynamicContextBank:
    ...
    def get_hint(self, label, url):
        ...
        if firm_hints: return random.choice(firm_hints)
        global_hints = self.global_bank.get(label)
        if global_hints: return random.choice(global_hints)
        return ""
...
def get_context_window(sentences, target_idx, window_size=1, override_target=None, injected_hint=None):
    start = max(0, target_idx - window_size)
    end = min(len(sentences), target_idx + window_size + 1)
    parts = []
    if injected_hint: parts.append(f"[Context: {injected_hint}]") 
...
def process_chunk(chunk_data):
    ...
    for url, matches_json in chunk_data:
        ...
        for para in paragraphs:
            if "<TABLE>" in para: continue
            
            # Optimization: Only process paragraphs that have ANY signal
            if not (STRICT_REGEX.search(para) or HEDGING_CONTEXT_REGEX.search(para) or EXCLUDE_REGEX_ACCOUNTING_STD.search(para)):
                continue

            sentences = [s.strip() for s in SENTENCE_SPLIT_PATTERN.split(para) if len(s.strip()) >= MIN_SENTENCE_LENGTH]
            
            for i, sentence in enumerate(sentences):
                match = STRICT_REGEX.search(sentence)
                
                # ======================================================
                # BRANCH A: SENTENCE CONTAINS "SWAP", "OPTION", ETC.
                # ======================================================
                if match:
                    ...
                    if len(specific_cats) == 1:
                        ...
                        blanked_target = target_sent[:match.start()] + "       " + target_sent[match.end():]
                        validation_text = get_context_window(sentences, i, CONTEXT_WINDOW_SIZE, override_target=blanked_target)
                        
                        score = -1 if has_conflict(validation_text, label) else scorer.score(validation_text, label)

                        local_candidates.append({
                            'label': label, 'sentences': sentences, 'target_idx': i,
                            'match_span': match.span(), 'match_text': match.group(0),
                            'original_sent': sentence, 'score': score, 'url': url
                        })
                        if score >= 20:
                            aug_text, _ = augmenter_dummy.augment(sentence, match.span(), match.group(0))
                            teacher_text = get_context_window(sentences, i, CONTEXT_WINDOW_SIZE, override_target=aug_text)
                            local_teachers.append((label, url, teacher_text))

                    elif len(specific_cats) == 0:
                        # Ambiguous Instrument ("We hold derivatives")
                        full_window = get_context_window(sentences, i, CONTEXT_WINDOW_SIZE)
                        # SAFETY: Ensure it doesn't contain context for specific classes
                        max_score = scorer.get_max_score_any_category(full_window)
                        if max_score < 10: 
                            local_candidates.append({
                                'label': 'gen', 'sentences': sentences, 'target_idx': i,
                                'match_span': match.span(), 'match_text': match.group(0),
                                'original_sent': sentence, 'score': 0, 'url': url,
                                'subtype': 'L0_Ambiguous_Instrument'
                            })

                # ======================================================
                # BRANCH B: NO INSTRUMENT NAME (Mining for Boilerplate)
                # ======================================================
                else:
                    ...
                    if is_hedging_talk or is_accounting:
                        ...
                        if len(specific_cats) == 0:
                            # NEW SAFETY: Check context score even for boilerplate
                            full_window = get_context_window(sentences, i, CONTEXT_WINDOW_SIZE)
                            max_score = scorer.get_max_score_any_category(full_window)
                            
                            if max_score < 10: # Only accept if context is truly generic
                                subtype = 'L0_Accounting' if is_accounting else 'L0_Risk_Policy'
                                local_candidates.append({
                                    'label': 'gen', 'sentences': sentences, 'target_idx': i,
                                    'match_span': None, 'match_text': "None",
                                    'original_sent': sentence, 'score': 0, 'url': url,
                                    'subtype': subtype
                                })
...
def create_labeled_dataset():
    print(f"🚀 Starting Dataset Generation v14 (Pure DB Source)")
        ...
        with tqdm(total=total_rows, desc="Scanning DB", unit="rows") as pbar:
            while active or futures:
                while len(futures) < MAX_WORKERS * 2 and active:
                    ...
                    except StopIteration: active = False
                ...
                for future in done:
                    ...
                    try:
                        ...
                        pbar.set_postfix({k: len(v) for k, v in global_candidates.items()})
                    except Exception as e: pbar.write(f"Error: {e}")
    conn.close()
    ...
    for label, items in global_candidates.items():
        if label == 'gen': random.shuffle(items)
        else: items.sort(key=lambda x: x['score'], reverse=True)
        
        for item in tqdm(items, desc=f"Generating {label}", leave=False):
            if stats[label] >= TARGET_SAMPLES_PER_CLASS: break
            ...
            row = {
                'text': '', 'label': label, 'difficulty': '',
                'debug_original': orig, 'debug_score': score, 'debug_hint': 'None'
            }
            ...
            if label == 'gen':
                ...
                if random.random() < 0.2:
                    ...
                    if hint:
                        row['text'] = get_context_window(sentences, idx, CONTEXT_WINDOW_SIZE, override_target=orig, injected_hint=hint)
                        row['difficulty'] = 'L4_Synthetic_Adverse'
                    else:
                        aug, _ = augmenter.augment(orig, match_span, match_text)
                        row['text'] = get_context_window(sentences, idx, CONTEXT_WINDOW_SIZE, override_target=aug)
                ...
                if random.random() < 0.5:
                    ...
                    if hint:
                        row['text'] = get_context_window(sentences, idx, CONTEXT_WINDOW_SIZE, override_target=aug, injected_hint=hint)
