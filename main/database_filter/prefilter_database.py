...
def count_information(text: str) -> dict:
    ...
    cr_regex = re.compile(r"|".join([CR_REGEX.pattern]), re.IGNORECASE)
    crypto_regex = re.compile(r"|".join([CRYPTO_REGEX.pattern]), re.IGNORECASE)
    misc_regex = re.compile(r"|".join([MISC_REGEX.pattern]), re.IGNORECASE)
    trading_regex = re.compile(r"|".join([TRADING_REGEX.pattern]), re.IGNORECASE)
...
def process_item(item: Tuple) -> Optional[Tuple]:
    ...
    try:
        ...
        is_nst_warr = (explicit_non_derivative_warr or not is_valid) if has_warr else True # default true to avoid future matches
        is_nst_conv = (explicit_non_derivative_conv or not is_valid) if has_conv else True # default true to avoid future matches
