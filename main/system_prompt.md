...
4.  **Detail Active Derivatives**: Populate the `active_derivatives` array. List each unique, currently active derivative instrument. Consolidate aggregate mentions (e.g., "various interest rate swaps") into a single entry. Prefer the current notional amount and currency in USD if there are redundant mentions.
