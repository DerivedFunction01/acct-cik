...
# Get system config as before, but use config file for NUM_THREADS
def get_system_config():
    ...
    print(
        f"⚙️  Configuration: TEXT_SIZE={text_size}"
    )
    return text_size

TEXT_SIZE = get_system_config()
...
def get_result_from_server(text_chunk: str) -> dict:
    ...
    Simple version: Nginx handles queuing automatically.
    """
    ...
    try:
        ...
        response = requests.post(
            predict_url, headers=headers, json=payload, timeout=600, stream=True
        )
        response.raise_for_status()
        ...
        return {"text": full_response, "success": True}

    except requests.exceptions.Timeout:
        print(f"❌ Timeout error after 600s")
        return {
            "error": "timeout",
