...
def generate_dataset2(
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in tqdm(futures, desc="Generating Samples"):
            narrative, target_json = future.result()
            # For this dataset version, we only want the narrative text as training data
            # Strip out any <reportingYear> tags for cleaner text
            narrative = re.sub(r'<reportingYear>\d+</reportingYear>', '', narrative).strip()
            # Only pick 1-3 paragraphs from the narrative to simulate partial text
            narrative_paragraphs = [p.strip() for p in narrative.split("\n\n") if p.strip()]
            num_paragraphs_to_select = random.randint(1, min(3, len(narrative_paragraphs)))
            # Make sure they are consecutive paragraphs
            start_index = random.randint(0, len(narrative_paragraphs) - num_paragraphs_to_select)
            selected_paragraphs = narrative_paragraphs[start_index:start_index + num_paragraphs_to_select]
            partial_narrative = "\n\n".join(selected_paragraphs)
            all_training_records.append(partial_narrative)

    df = pd.DataFrame(all_training_records, columns=["text"])
    df.to_parquet(output_file, index=False)
