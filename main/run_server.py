...
def start_servers():
    ...
    if is_windows():
        ...
        waitress_cmd = f"waitress-serve --host 127.0.0.1 --port {NGINX_PORT} {SERVER_SCRIPT}"
...
def stop_servers():
    ...
    if is_windows():
        print("ℹ️  On Windows, the server runs in the foreground. To stop it, press Ctrl+C in the terminal where it is running.")
        return
